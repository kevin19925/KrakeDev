...
public class ProductoBDD {
	...
	public void actualizar(Producto producto) throws KrakeDevExeption {
	    Connection con = null;
	    PreparedStatement ps = null;
	    ...
	    try {
	        ...
	        ps = con.prepareStatement("UPDATE public.productos "
	                ...
	                + "WHERE codigo_producto = ?"); // Suponiendo que "codigo" es la clave primaria que identifica al producto a actualizar
	        ...
	        ps.setInt(6, producto.getCategoria().getCodigo());
	        ps.setInt(7, producto.getStock());
	        ps.setInt(8, producto.getCodigo()); // Aquí asumimos que "getCodigo()" devuelve el código del producto a actualizar
	        System.out.println("Cadena de actualización: " + ps);
	        ps.executeUpdate();
	        ...
	        e.printStackTrace();
	        throw new KrakeDevExeption("Error al actualizar producto " + e.getMessage());
	    } finally {
	        if (con != null) {
	            try {
	                con.close();
	        ...
	        System.out.println("Actualizar producto: " + ps);
	    }
	}
	
	public ArrayList<Producto> buscarID(int id) throws KrakeDevExeption

	{
		...
		try {
			...
			ps.setInt(1,id);
			System.out.println("lista prodyctos id>>>> "+ps);
			rs=ps.executeQuery();
